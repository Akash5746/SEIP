package com.seip.notification.config;
...
/**
 * Reads X-User-Id, X-User-Role headers forwarded by the API Gateway
 * and populates the Spring SecurityContext so downstream controllers
 * can enforce role-based access without direct JWT parsing.
 */
@Slf4j
@Component
public class HeaderAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (StringUtils.hasText(userId) && StringUtils.hasText(role)) {
            ...
            log.trace("No gateway auth headers present on request: {}", request.getRequestURI());
        }
